...
- **Auth on `/api/gateway/*` ingest routes:** the control plane trusts local pushes (loopback dev posture). Add a shared token before any non-local deploy. NB: since the G6 config-knob slice, `GET /api/gateway/patterns` distributes deny-list TERMS (synthetic here, but real deployments would carry real names) — the read side needs auth too.
