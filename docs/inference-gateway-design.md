...
> *Claude Opus 4.8 (xhigh effort) says:*
>
> ***Can we build it?*** *Yes — and that isn't a courtesy. The team reading this already shipped the control plane this gateway bolts onto: sixteen vendor adapters, a policy-and-drift engine, a synthetic surface layer, a green test suite. The gateway is the smaller, more contained problem. Its core — a reverse proxy that rewrites JSON and swaps a credential — is an afternoon; the genuinely hard parts are narrow and well-understood (streaming correctness, and detection recall via a regex floor plus a real NER/DLP engine). Neither is research. And the asks that shaped this document — keep it off the spine, tee don't gate, bind tokens so the oracle works, surrogate before the corpus — are the right questions in the right order. The capability is not in doubt.*
>
> ***Should we build it?*** *Not by default — on demand. This is the optional module for real reasons: it makes you latency-critical, a single point of failure, and the owner of per-vendor request-shape upkeep and a live PHI path. The spine — driving vendor admin APIs and pushing managed config — already delivers most governance with none of that cost, which is exactly why the gateway sits off the critical path. So build it when a requirement lands that genuinely needs in-request enforcement: a customer who must scrub PHI before it reaches the model; a team that must **technically enforce a pre-coverage gate** — "no PHI until ZDR is on" — rather than police it by policy, then minimize incidental PHI once covered; or a hard cross-vendor spend cut no vendor will do natively. When that need is real, the differentiator — a published recall ratchet on top of dual control — is worth more than anything the proxy-only incumbents ship. Until then, the honest move is to keep it on the shelf: designed, costed, ready. Build it when the need is real, and you'll build it well.*
