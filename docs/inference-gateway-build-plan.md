...
**Status:** MVP slice **built** (2026-07-02): G0, G1, G2, partial-G6 (self-hosted Presidio tier), G7, plus a UI test harness tab and flywheel v0 — see [`plans/phase-3-inference-gateway.md`](./plans/phase-3-inference-gateway.md) for what shipped and [`plans/TODO-gateway-deferred.md`](./plans/TODO-gateway-deferred.md) for what's deliberately deferred (G3–G5, managed DLP, G8–G10-full). `cmd/air-traffic-gateway` runs on 127.0.0.1:8125; Presidio sidecar on 8126.
