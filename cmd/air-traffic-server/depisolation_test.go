...
func TestServerClosureExcludesGatewayAndThirdParty(t *testing.T) {
	out, err := exec.Command("go", "list", "-deps", "air-traffic/cmd/air-traffic-server").CombinedOutput()
	if err != nil {
		t.Fatalf("go list -deps: %v\n%s", err, out)
	}
	for _, dep := range strings.Fields(string(out)) {
		if strings.HasPrefix(dep, "air-traffic/internal/gateway") {
			t.Errorf("control-plane closure links gateway package %s", dep)
		}
		if strings.HasPrefix(dep, "air-traffic/") || strings.HasPrefix(dep, "vendor/") {
